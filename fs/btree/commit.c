				     ...
				     struct btree_path *path, unsigned u64s)
{
	...
	if (u64s > ck->u64s) {
		...
		if (unlikely(!new_k)) {
			...
			new_k = kmalloc(new_u64s * sizeof(u64), GFP_KERNEL|__GFP_RECLAIMABLE|__GFP_NOWARN);
			if (!new_k) {
				struct bch_fs *c = trans->c;
...
static int bch2_trans_commit_run_triggers(struct btree_trans *trans)
{
	...
	while (sort_id_start < trans->nr_updates) {
		unsigned idx, sort_id = trans->updates[sort_id_start].sort_order;
			...
			bool lazy)
{
	...
	trans_for_each_update(trans, i) {
		...
		*dst++ = *i;
		...
		if (unlikely(ret)) {
			trans_commit_compact_tail(trans, dst, i);
			goto out_reset;
		}
		...
		if (!i->cached) {
			...
			if (!same_leaf_as_next(trans, i) &&
				...
				if (unlikely(btree_node_needs_merge(c, b, u64s_delta))) {
					ret = trans_commit_merge(trans, flags, &i, &dst);
					if (ret)
						goto out_reset;
				}
